 ...
 * Component: Core / Tests / Deserialization
...
fn test_should_deserialize_sample_metadata_when_valid_json_provided() {
    ...
    let metadata: PharosMetadata =
        serde_json::from_str(data).expect("Failed to deserialize sample");

    assert_eq!(metadata.metadata_id, "PHX-DW-001");
    assert_eq!(
        metadata.parameters.get("PKD_Manufacturer").unwrap(),
        &ParameterValue::Text("Pharos Kitchen Systems".to_string())
    );
}
