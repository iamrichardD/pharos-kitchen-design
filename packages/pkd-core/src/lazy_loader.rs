...
impl LazyShardLoader {
    ...
    pub fn new(base_url: String, manifest: HashMap<String, String>, fetcher: Arc<dyn ShardFetcher>) -> Self {
        Self { base_url, manifest, fetcher }
    }
    ...
    pub async fn load_shard(&self, shard_id: &str) -> Result<RegistryShard> {
        let expected_hash = self.manifest.get(shard_id)
            .ok_or_else(|| anyhow!("[Security] Shard ID {} not found in authoritative manifest.", shard_id))?;

        let shard_url = format!("{}/shard_{}.json", self.base_url, shard_id);
        
        let bytes = self.fetcher.fetch(shard_url).await
        ...
        verify_bytes(&bytes, expected_hash)
            .map_err(|e| anyhow!("[Security] Integrity check failed for shard {}: {}", shard_id, e))?;
...
mod tests {
    ...
    impl ShardFetcher for MockFetcher {
        fn fetch(&self, url: String) -> Pin<Box<dyn Future<Output = Result<Vec<u8>>> + Send>> {
            let res = self.data.get(&url).cloned()
    ...
    async fn test_should_load_shard_when_valid_manifest_and_data() {
        ...
        data.insert(format!("https://cdn.example.com/shard_{}.json", shard_id), shard_bytes);
    ...
    async fn test_should_fail_fast_when_network_error() {
        ...
        let fetcher = Arc::new(MockFetcher { data: HashMap::new() });
        let loader = LazyShardLoader::new("https://cdn.example.com".to_string(), manifest, fetcher);
