# @TODO: Pharos Kitchen Design (Project Prism)

## 📋 Backlog

### Phase 1: Marketing & Foundation
- [ ] Implement Playwright interaction screenshots for marketing site.
- [ ] Implement instructional content for "Ghost-Link" on `/bridge`.
- [ ] Enhance the Maturity Matrix with live crawler data integration.

### Phase 2: Metadata Core & Identity (Project Prism)
- [x] Issue #5: Provision AWS Cognito & Cloudflare D1 (OpenTofu).
- [x] Issue #6: Establish GitHub-to-AWS OIDC Federation (Security).
- [ ] Issue #7: Integrate Auth Bridge with Live Cognito (Real JWTs).
- [ ] Implement `keyring-rs` integration in Rust CLI for secure token storage.
- [ ] Develop the "Hero" protocol for manufacturer verification (ABAC).

### Phase 3: WebMCP & Agentic AI
- [ ] Implement the WebMCP interface for AI agent equipment queries.
- [ ] Develop agentic selection logic for "Smart Specification."

### Phase 4: Revit & Interop
- [ ] Finalize the `pkd-bridge` WASM core for cross-platform Revit targets.
- [ ] Implement the Pharos Spotlight Search (Cmd+K) in the Revit plugin.
