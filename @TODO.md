...
- [ ] **Issue #314**: [TAG: Core] Support URL-Driven User Code and Optional Workspace Pairing. [DESC: Implement URL query parameter binding for verify page to pre-populate user code and dynamically toggling field visibility based on referrer.] [ECT: 2]
- [ ] **Issue #315**: [TAG: Core] Implement URL Query Parameter Binding for Revit Plugin Verify Flow. [DESC: Enable Revit plugin action gate to append user_code to redirect URL for automatic verification path.] [ECT: 1]
- [ ] **Issue #316**: [TAG: Core] Implement URL Query Parameter Binding for CLI Verify Flow. [DESC: Enable Pharos CLI tool to append user_code to redirect URL for seamless command-line device authentication.] [ECT: 1]
